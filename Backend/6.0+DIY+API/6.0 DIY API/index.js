...
app.get("/jokes/:id", (req,res)=>{
  ...
  const specJokeId = jokes.find((joke) => joke.id === id);
  // let specJokeId;
...
app.patch("/jokes/:id",(req,res)=>{
  ...
  if (req.body.type){
    jokes[searchIdx].jokeType = req.body.type;
  }
  if (req.body.text){
    jokes[searchIdx].jokeText = req.body.text;
  }
  console.log(jokes[searchIdx]);
...
app.delete("/all",(req,res)=>{
  ...
  if(userKey===masterKey){
    ...
    res.status(404).json({error:`You are not authoriseed to perform this action.`});
  }
});
